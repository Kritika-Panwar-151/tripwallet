...
**21,751 rows across 15 tables**, of which 13 are the tables your
statement is actually built on. The rest are small reference tables the others point at.

## Ninety seconds to your first query

```bash
sqlite3 data/PS-08.db < data/queries/starter_queries.sql
```

Or open `data/PS-08.db` in any SQLite client. Nothing to install, nothing to import.
...
In `PS-08.db` the money columns are declared `TEXT`. That is deliberate: SQLite's NUMERIC
...
```bash
python3 tools/validate_conformance.py data/PS-08.db     # or point it at your own export
```
